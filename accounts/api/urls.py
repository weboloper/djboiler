...
urlpatterns = [
    ...
    path("password/", include([
        path('reset/', password_views.ResetPasswordRequestAPIView.as_view(), name='password_reset_request'),
        path('reset/<uidb64>/<token>/', password_views.ResetPasswordConfirmAPIView.as_view(), name='password_reset_confirm'),
        path('change/', password_views.ChangePasswordAPIView.as_view(), name='password_change'),

    ])),

    # email urls
    path("email/", include([
        path('verify/', email_views.EmailVerificationRequestAPIView.as_view(), name='email_verify_request'),
        path('verify/<uidb64>/<token>/', email_views.EmailVerificationConfirmAPIView.as_view(), name='email_verify_confirm'),
        path('change/', email_views.EmailChangeRequestView.as_view(), name='email_change_request'),
        path('change/<uidb64>/<token>/', email_views.EmailChangeConfirmView.as_view(), name='email_change_confirm'),
    ])),

    path('google_auth/', oauth_views.GoogleAuth.as_view(), name='google_auth'),
    path('google_auth_callback/', oauth_views.GoogleAuthCallback.as_view(), name='google_auth_callback'),  # add path for google authentication
